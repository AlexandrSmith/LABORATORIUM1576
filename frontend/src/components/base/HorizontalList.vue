...
export default Vue.extend({
  ...
  methods: {
    ...
    scrollToIndex(i: number): void {
      ...
      if (slots[i]) {
        const container = this.$refs.container as Element
        const elem = slots[i].elm as Element
        ...
        //   width: this.width,
        //   rectLeft: rect.left,
        //   boundingRect: container.getBoundingClientRect().left,
        // })
    ...
    refresh(callback: (data: HorizontalData) => void): void {
      this.$nextTick(() => {
    ...
    calculate(): HorizontalData {
      const container = this.$refs.container as Element
      const slots = this.slots()
      const slot0 = slots[0]?.elm as Element
